...
import io.boodskap.iot.camera.Camera;
import io.boodskap.iot.camera.CameraConfig;
import io.boodskap.iot.camera.ImageEncoding;
import io.boodskap.iot.camera.RPICameraConfig;
 ...
 */
public class RPiCamera extends Camera {
	...
	public RPiCamera(CameraConfig config) throws FailedToRunRaspistillException{
		
		this();
		
		this.config = config;
		
		RPICameraConfig rpic = config.getRpi();
		
		this.setWidth(config.getImageWidth());
		this.setHeight(config.getImageHeight());
		this.setFullPreviewOff();
		this.setQuality(config.getImageQuality());
		
		
		if(rpic.getAwb() != AWB.NONE) {
			this.setAWB(rpic.getAwb());
		}
		
		if(rpic.getDrc() != DRC.NONE) {
			this.setDRC(rpic.getDrc());
		}
		
		if(rpic.getExposure() != Exposure.NONE) {
			this.setExposure(rpic.getExposure());
		}
		
		if(rpic.getImageEffect() != ImageEffect.NONE) {
			this.setImageEffect(rpic.getImageEffect());
		}
		
		if(rpic.getMeteringMode() != MeteringMode.NONE) {
			this.setMeteringMode(rpic.getMeteringMode());
		}
		
		if(rpic.getRotation() != Rotation.NONE) {
			this.setRotation(rpic.getRotation().value());
		}

		int[] ceff = rpic.getColorEffect();
		
		this.selectCamera(rpic.getCameraId());
		this.setBrightness(rpic.getBrightness());
		if(rpic.getIso() != -1) this.setISO(rpic.getIso());
		if(rpic.getSaturation() != -1) this.setSaturation(rpic.getSaturation());
		if(rpic.getContrast() != 0) this.setContrast(rpic.getContrast());
		if(rpic.getSharpness() != 0) this.setSharpness(rpic.getSharpness());
		if(rpic.getShutterSpeed() != -1) this.setShutter(rpic.getShutterSpeed());
		if(rpic.isVerticalFlip()) this.setVerticalFlipOn();
		if(rpic.isHorizontalFlip()) this.setHorizontalFlipOn();
		if(ceff[0] != -1 && ceff[1] != -1) this.setColourEffect(ceff[0], ceff[1]);
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	 ...
	 */
	public RPiCamera setEncoding(ImageEncoding encoding) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public String getName() {
		return String.format("Cam%d", config.getRpi().getCameraId());
	}

	@Override
	public BufferedImage getImage() throws IOException, InterruptedException {
		return this.takeBufferedStill();
	}
}
